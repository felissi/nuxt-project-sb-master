...
app.bootstrap({
  ...
  // logLevel: 'Error',
  // sort: ['source-order'],
  exclude: ["node_modules", "components/lib/**/*.js", "**/*.test.ts"],
  plugin: [],
});
...
if (project) {
  ...
  project.children.forEach((module) => {
    ...
    console.log(`🚀 // DEBUG 🍔 ~ file: build-apidoc.cjs:509 ~ module.groups:`, module);
    console.log(
      `🚀 // DEBUG 🍔 --------------------------------------------------------------------------🚀 // DEBUG 🍔`
    );

    const module_component_group = module?.group?.find((g) => g.title === "Component");
    let methods = {
    ...
    module_component_group &&
      module_component_group.children.forEach((component) => {
        ...
        const component_method_group =
          component.groups && component?.group?.find((g) => g.title === "Methods");
        ...
        const component_props_id =
          component.extendedTypes &&
          component.extendedTypes[0].typeArguments &&
          component.extendedTypes[0].typeArguments[0] &&
          component.extendedTypes[0].typeArguments[0]._target;
        const module_properties_group = module?.group?.find((g) => g.title === "Properties");
        const component_props =
          module_properties_group &&
          module_properties_group.children.find((c) =>
            component_props_id ? c.id === component_props_id : true
          );
        ...
        if (component_props) {
          props.description = component_props.comment
            ? component_props.comment.summary.map((s) => parseText(s.text || "")).join(" ")
            : "";

          const component_props_group =
            component_props.groups && component_props?.group?.find((g) => g.title === "Properties");

          component_props_group &&
            component_props_group.children.forEach((prop) => {
              if (
                !prop.inheritedFrom ||
                (prop.inheritedFrom && !prop.inheritedFrom.toString().startsWith("Omit.data-pr-"))
              ) {
                props.values.push({
                  name: prop.name,
                  ...
                  default:
                    prop.comment && prop.comment.getTag("@defaultValue")
                      ? parseText(prop.comment.getTag("@defaultValue").content[0].text)
                      : "", // TODO: Check
                  description:
                    prop.comment &&
                    prop.comment.summary.map((s) => parseText(s.text || "")).join(" "),
                  deprecated:
          ...
          const component_props_methods_group =
            component_props.groups && component_props?.group?.find((g) => g.title === "Methods");
              ...
              methods.values.push({
                ...
                parameters: signature.parameters.map((param) => {
                  return {
                    ...
                    description:
                      param.comment &&
                      param.comment.summary.map((s) => parseText(s.text || "")).join(" "),
                  };
                }),
                returnType: signature.type.toString(),
                description: signature.comment.summary
                  .map((s) => parseText(s.text || ""))
                  .join(" "),
              });
            });
        }
    ...
    const module_model_group = module?.group?.find((g) => g.title === "Model");

    module_model_group &&
      module_model_group.children.forEach((model) => {
        const event_props_description =
          model.comment && model.comment.summary.map((s) => s.text || "").join(" ");
        ...
        const model_props_group = model?.group?.find((g) => g.title === "Properties");
        ...
        const model_methods_group = model?.group?.find((g) => g.title === "Methods");
        ...
        model_methods_group &&
          model_methods_group.children.forEach((method) => {
            ...
            methods.values.push({
                  ...
                  if (param.type.declaration.children) {
                    param.type.declaration.children.forEach((child) => {
                      if (child.signatures) {
                        ...
                        const parameters = childSinature.parameters.reduce(
                          (acc, { name, type }, index) =>
              ...
              description:
                signature.comment &&
                signature.comment.summary.map((s) => parseText(s.text || "")).join(" "),
              deprecated:
                signature.comment && signature.comment.getTag("@deprecated")
                  ? parseText(signature.comment.getTag("@deprecated").content[0].text)
                  : undefined,
            });
          });
    ...
    const module_functions_group = module?.group?.find((g) => g.title === "Functions");
    ...
    const module_events_group = module?.group?.find((g) => g.title === "Events");
    ...
    module_events_group &&
      module_events_group.children.forEach((event) => {
        ...
        const component_prop =
          event.comment && event.comment.getTag("@see")
            ? event.comment.getTag("@see").content[0].text
            : ""; // TODO: Check
        ...
        const props = [];
        const event_props_group = event?.group?.find((g) => g.title === "Properties");
    ...
    const module_interfaces_group = module?.group?.find((g) => g.title === "Interfaces");
      ...
      module_interfaces_group.children.forEach((event) => {
        ...
        if (event.comment && event.comment.getTag("@see")) {
          ...
          if (content.text.includes("['")) {
            ...
            component_prop = `${
              content.text === content.target?.name
                ? content.target.parent.name
                : content.target?.name
            }.${content.text}`;
          }
        }
        ...
        if (event.groups) {
                    ...
                    .map((s) => {
                      if (s.text.indexOf("[here]") > -1) {
                        return `${s.text.slice(
                          0,
                          s.text.indexOf("[here]")
                        )} <a target="_blank" href="${s.text.slice(
                          s.text.indexOf("(") + 1,
                          s.text.indexOf(")")
                        )}">here</a> ${s.text.slice(s.text.indexOf(")") + 1)}`;
                      }
          ...
          const event_methods_group = event?.group?.find((g) => g.title === "Methods");
                      ...
                      param.type.declaration.children.forEach((child) => {
                        if (child.signatures) {
                          ...
                          if (child.type?.declaration?.signatures) {
                            ...
                            child.type?.declaration?.signatures[0]?.parameters.map(
                              (param, index) => {
                            ...
                            if (
                              child.type?.declaration?.signatures[0]?.comment?.getTag("@deprecated")
                                ?.content[0]?.text
                            ) {
        ...
        if (signature && signature.length > 0) {
          ...
          props.push({
            ...
            description:
              signature[0].comment &&
              signature[0].comment.summary.map((s) => s.text || "").join(" "),
          });
        }
    ...
    const module_types_group = module?.group?.find((g) => g.title === "Type Aliases");
    ...
    module_types_group &&
      module_types_group.children.forEach((event) => {
        ...
        if (values.includes("Function") && event.type.types) {
          ...
          for (const [i, type] of event.type.types.entries()) {
            if (type.declaration && type.declaration.signatures) {
              ...
              const parameters = signature.parameters.reduce(
                (acc, { name, type }, index) =>
                  index === 0 ? `${name}: ${type.name}` : `${acc}, ${name}: ${type.name}`,
                ""
              );
              ...
              values +=
                i === 0
                  ? `(${parameters}) => ${signature.type?.name}`
                  : ` | (${parameters}) => ${signature.type?.name}`;
            } else {
        ...
        if (declaration) {
          const groups =
            declaration.groups && declaration?.group?.find((g) => g.title === "Properties");
          ...
          groups &&
            groups.children.forEach((prop) => {
              const description =
                prop.comment && prop.comment.summary.map((s) => s.text || "").join(" ");

              map[`${prop.name}${prop.flags.isOptional ? "?" : ""}`] = `${prop.type.toString()}, ${
  ...
  app.generateDocs(project, `./api-generator/docs/`);
}
